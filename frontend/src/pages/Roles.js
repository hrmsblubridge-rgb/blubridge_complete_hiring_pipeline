...
export default function Applicants() {
    ...
    const [dateType, setDateType] = useState('Registered');
    // iter108 — Default current day filter so View Applicants loads only
    // today's records on initial mount. Reset reverts to "all history".
    ...
    // iter123 — Trigger CSV/XLSX export honouring all currently-applied filters.
    const doExport = async (format) => {
        ...
        try {
            ...
            const res = await axios.get(`${API}/api/applicants/export?${params.toString()}`,
            ...
            a.href = url;
            a.download = `View_Applicants_${dayjs().format('YYYY-MM-DD')}.${format}`;
            a.click();
        ...
        <div className="min-h-screen bg-[#0a0a0a] text-white" data-testid="applicants-page">
            ...
            <div className="px-8 py-5 border-b border-zinc-800" data-testid="filters-section">
                <div className="flex flex-wrap items-end gap-4">
                    ...
                    <div className="space-y-1.5">
                        <label className="text-xs text-zinc-500 uppercase tracking-wider">Date Filter Type</label>
                        <select value={dateType} onChange={e => setDateType(e.target.value)} data-testid="date-type-filter"
            ...
            <div className="px-8 py-6">
                {loading ? (
                    ...
                    <>
                        <div className="overflow-x-auto border border-zinc-800" data-testid="applicants-table">
