...
async def lookup_applicant(request: Request, email: Optional[str] = None, phone: Optional[str] = None):
    ...
    return {
        ...
        # iter110 — Prefer the persisted bucketed status to fix legacy "-" display.
        "college_type":    rec.get("_college_status") or rec.get("college_type") or "",
...
async def manual_otp_verify(body: ManualVerifyBody, request: Request):
    ...
    return {
        ...
        "applicant": {
            ...
            "college_type":  full.get("_college_status") or full.get("college_type") or "",
